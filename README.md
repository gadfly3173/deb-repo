...
通过 GitHub Actions 自动构建和维护的 Debian APT 仓库。支持从推送的 .deb 包和上游 GitHub/GitLab Release 自动同步。
...
在仓库根目录编辑此文件，定义需要同步的上游仓库（支持 GitHub 和 GitLab）：
...
[
  ...
  },
  {
    ...
    "pattern": "glab_*_linux_amd64.deb"
  },
  {
    "provider": "gitlab",
    "project": "gitlab-org/cli",
    "pattern": "glab_*_linux_arm64.deb"
  },
  {
    "owner": "reqable",
    "repo": "reqable-app",
    "pattern": "reqable-app-linux-x86_64.deb"
  }
]
```
...
| 字段 | 必填 | 说明 |
|------|------|------|
| `provider` | 否 | 上游平台，`github`（默认）或 `gitlab` |
| `owner` | GitHub 必填 / GitLab 可选 | GitHub 用户名或组织名；GitLab 可与 `repo` 组合为项目路径 |
| `repo` | GitHub 必填 / GitLab 可选 | GitHub 仓库名；GitLab 可与 `owner` 组合为项目路径 |
| `project` | GitLab 推荐 | GitLab 项目路径（如 `gitlab-org/cli`，支持子组路径） |
| `pattern` | 否 | 文件匹配模式（支持 `*` 和 `?`），默认 `*.deb` |

示例：
- `*.deb`：同步该 Release 下所有 `.deb`
- `reqable-app-linux-x86_64.deb`：只同步指定文件名（适用于 Reqable）
- `glab_*_linux_amd64.deb`：同步 glab 各版本的 amd64 包
- `glab_*_linux_arm64.deb`：同步 glab 各版本的 arm64 包
...
> 如需同时同步多个文件名，可为同一个 `project` 添加多条配置。
...
| `GL_TOKEN` | 否 | GitLab API Token（访问私有项目时需要；公开项目可不填） |
